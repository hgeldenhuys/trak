...
export interface StartSessionInput {
  /** Actor who is starting the session */
  actor: string;
  /** Optional active story ID */
 ...
 */
function getChangedFields(previous: Session, current: Session): (keyof Session)[] {
  const fields: (keyof Session)[] = [];
  const keys = Object.keys(current) as (keyof Session)[];
...
export class SessionRepository {
   ...
   */
  findByActor(actor: string): Session[] {
    const db = getDb();
    const rows = db
      .query(
        `SELECT * FROM ${TABLES.SESSIONS}
  ...
  update(id: string, input: UpdateSessionInput): Session {
    ...
    const stmt = db.prepare(
      `UPDATE ${TABLES.SESSIONS} SET ${updates.join(', ')} WHERE id = ?`
    );
    stmt.run(...values);
   ...
   * @emits session:updated
   */
  incrementCompactionCount(id: string): Session {
    const db = getDb();
    const previous = this.findById(id);
   ...
   */
  endAllActive(): Session[] {
    const activeSessions = this.findAllActive();
    const endedSessions: Session[] = [];
