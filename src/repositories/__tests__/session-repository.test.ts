...
import type { SessionStartedEvent, SessionEndedEvent, SessionUpdatedEvent } from '../../events';
...
describe('SessionRepository', () => {
  ...
  describe('start', () => {
    ...
    it('should emit session:started event', () => {
      ...
      const events = capturedEvents.filter(
        (e) => (e as SessionStartedEvent).entityId === session.id
      );
  ...
  describe('update', () => {
    ...
    it('should emit session:updated event', () => {
      ...
      const updateEvents = capturedEvents.slice(startEventCount).filter(
        (e) => (e as SessionUpdatedEvent).entityId === created.id
      );
      expect(updateEvents.length).toBeGreaterThanOrEqual(1);
      const event = updateEvents[0] as SessionUpdatedEvent;
  ...
  describe('end', () => {
    ...
    it('should emit session:ended event with duration', () => {
      ...
      const endEvents = capturedEvents.slice(startEventCount).filter(
        (e) => (e as SessionEndedEvent).entityId === created.id
      );
      expect(endEvents.length).toBeGreaterThanOrEqual(1);
      const event = endEvents[0] as SessionEndedEvent;
      expect(event.entityId).toBe(created.id);
