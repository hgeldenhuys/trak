 ...
 */
function generateStoryCode(featureCode: string, counter: number): string {
...
function toStory(row: Record<string, unknown>): Story {
  return {
    ...
    estimatedComplexity: row.estimated_complexity as string | null,
...
export class StoryRepository {
   ...
   */
  create(input: CreateStoryInput): Story {
    const db = getDb();
    ...
    const feature = db
      .query<Record<string, unknown>, [string]>(
        `SELECT * FROM ${TABLES.FEATURES} WHERE id = ?`
      )
      .get(input.featureId);
    ...
    const newCounter = (feature.story_counter as number) + 1;
    const code = generateStoryCode(feature.code as string, newCounter);

    // Update feature counter
    db.run(
      `UPDATE ${TABLES.FEATURES} SET story_counter = ?, updated_at = ? WHERE id = ?`,
      [newCounter, now, input.featureId]
    );

    // Insert the story
    db.run(
      `INSERT INTO ${TABLES.STORIES} (
        id, code, feature_id, title, description, why,
        status, priority, assigned_to, estimated_complexity, extensions,
      ...
      [
        ...
        input.estimatedComplexity ?? null,
  ...
  findById(id: string): Story | null {
    ...
    const row = db
      .query<Record<string, unknown>, [string]>(
        `SELECT * FROM ${TABLES.STORIES} WHERE id = ?`
      )
      .get(id);
   ...
   * @returns The story or null if not found
   */
  findByCode(code: string): Story | null {
    const db = getDb();
    const row = db
      .query<Record<string, unknown>, [string]>(
        `SELECT * FROM ${TABLES.STORIES} WHERE code = ?`
      )
      .get(code);
   ...
   */
  findByFeatureId(featureId: string): Story[] {
    const db = getDb();
    const rows = db
      .query<Record<string, unknown>, [string]>(
        `SELECT * FROM ${TABLES.STORIES} WHERE feature_id = ? ORDER BY created_at ASC`
      )
      .all(featureId);
   ...
   * @param options - Query options (excludeArchived defaults to true)
   * @returns Array of matching stories
   */
  findAll(
    filters?: { status?: StoryStatus; featureId?: string },
    ...
    // Default: exclude archived stories unless explicitly included
    ...
    const rows = db
      .query<Record<string, unknown>, (string | null)[]>(query)
      .all(...params);
   ...
   */
  update(id: string, input: UpdateStoryInput): Story {
    const db = getDb();
    const now = new Date().toISOString();
    ...
    const changedFields: (keyof Story)[] = [];
    ...
    if (input.estimatedComplexity !== undefined && input.estimatedComplexity !== previousState.estimatedComplexity) {
      updates.push('estimated_complexity = ?');
      params.push(input.estimatedComplexity);
      changedFields.push('estimatedComplexity');
    }
    ...
    if (changedFields.length > 0) {
      params.push(id);
      db.run(
        `UPDATE ${TABLES.STORIES} SET ${updates.join(', ')} WHERE id = ?`,
        params
      );
    }
   ...
   * @throws Error if story not found
   */
  updateStatus(id: string, newStatus: StoryStatus): Story {
    const db = getDb();
    const now = new Date().toISOString();
    ...
    // Update status
    db.run(
      `UPDATE ${TABLES.STORIES} SET status = ?, updated_at = ? WHERE id = ?`,
      [newStatus, now, id]
    );
