 ...
 */
function rowToEntity(row: AcceptanceCriteriaRow): AcceptanceCriteria {
  return {
    id: row.id,
...
function getChangedFields(
  ...
  const fields: (keyof AcceptanceCriteria)[] = [];
  const keys = Object.keys(current) as (keyof AcceptanceCriteria)[];
...
export class AcceptanceCriteriaRepository {
   ...
   * @emits ac:created
   */
  create(input: CreateAcceptanceCriteriaInput): AcceptanceCriteria {
    const db = getDb();
    ...
    db.run(
      `INSERT INTO ${TABLES.ACCEPTANCE_CRITERIA} (
   ...
   */
  findById(id: string): AcceptanceCriteria | null {
    const db = getDb();
    const row = db
      .query(`SELECT * FROM ${TABLES.ACCEPTANCE_CRITERIA} WHERE id = ?`)
      .get(id) as AcceptanceCriteriaRow | null;
   ...
   */
  findByStoryId(storyId: string): AcceptanceCriteria[] {
    const db = getDb();
    const rows = db
      .query(`SELECT * FROM ${TABLES.ACCEPTANCE_CRITERIA} WHERE story_id = ? ORDER BY code ASC`)
      .all(storyId) as AcceptanceCriteriaRow[];

    const entities: AcceptanceCriteria[] = [];
   ...
   * @emits ac:updated
   */
  update(id: string, input: UpdateAcceptanceCriteriaInput): AcceptanceCriteria {
    const db = getDb();
    ...
    const stmt = db.prepare(
      `UPDATE ${TABLES.ACCEPTANCE_CRITERIA} SET ${updates.join(', ')} WHERE id = ?`
    );
    stmt.run(...values);
   ...
   * @emits ac:verified
   */
  verify(id: string, notes: string): AcceptanceCriteria {
    const db = getDb();
    const previous = this.findById(id);
    ...
    db.run(
      `UPDATE ${TABLES.ACCEPTANCE_CRITERIA} SET
        status = ?,
        verification_notes = ?,
        verified_at = ?,
        updated_at = ?
      WHERE id = ?`,
      ['verified', notes, now, now, id]
    );
   ...
   * @emits ac:verified
   */
  fail(id: string, notes: string): AcceptanceCriteria {
    const db = getDb();
    const previous = this.findById(id);
   ...
   */
  countByStatus(storyId: string): { pending: number; verified: number; failed: number } {
    const db = getDb();
    const rows = db
      .query(
   ...
   */
  allVerified(storyId: string): boolean {
