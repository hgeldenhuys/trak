...
export function createStoryCommand(): Command {
  ...
  .option('-c, --complexity <level>', 'Estimated complexity (trivial, small, medium, large, epic)')
  .option('--draft', 'Create as local draft (not synced to ADO)')
  .action(async (options) => {
    try {
      ...
      // 2. Handle --draft flag: always set status to 'draft' regardless of --status option
      let status: StoryStatus;
      if (options.draft) {
        status = StoryStatus.DRAFT;
      } else {
        status = options.status.toLowerCase().replace(/-/g, '_') as StoryStatus;
      ...
      // For draft stories, explicitly set extensions without adoWorkItemId
      const extensions: Record<string, unknown> = options.draft
        ? { isDraft: true }
        : {};
      ...
      const story = storyRepository.create({
        ...
        status,
        priority,
        estimatedComplexity: options.complexity || null,
        extensions,
      });
      ...
      if (getOutputFormat() === 'json') {
        ...
        if (options.draft) {
          success(`Local draft story created: ${story.code}`);
          console.log('\x1b[33mThis story is a local draft and will not be synced to ADO until promoted.\x1b[0m');
        } else {
        ...
        output({
          ...
          Why: story.why || '(none)',
          ...(options.draft ? { 'Local Draft': 'Yes (not synced to ADO)' } : {}),
        });
      }
    } catch (err) {
...
storyCommand
  ...
  .action(async (code: string) => {
    try {
      ...
      // Get decisions
      const decisions = decisionRepository.findByEntity(EntityType.STORY, story.id);
      ...
      if (getOutputFormat() === 'json') {
        ...
        if (story.estimatedComplexity) {
          console.log(`\x1b[1mComplexity:\x1b[0m ${story.estimatedComplexity}`);
        }
        if (story.assignedTo) {
        ...
        console.log(`\x1b[1mAcceptance Criteria:\x1b[0m (${verifiedCount}/${criteria.length} verified)`);
        if (criteria.length === 0) {
          console.log('  (none defined)');
        } else {
          for (const ac of criteria) {
            const statusIcon = ac.status === 'verified' ? '\x1b[32m✓\x1b[0m' :
        ...
        if (tasks.length === 0) {
          ...
          console.log(formatTable(
            taskRows,
            ['order', 'status', 'priority', 'title', 'assignedTo'],
            {
              headers: {
                order: '#',
                status: 'STATUS',
        ...
        if (decisions.length > 0) {
          console.log(`\x1b[1mDecisions:\x1b[0m (${decisions.length})`);
          for (const d of decisions) {
            const statusColor = d.status === 'accepted' ? '\x1b[32m' :
                               d.status === 'superseded' ? '\x1b[35m' :
                               d.status === 'deprecated' ? '\x1b[90m' :
                               '\x1b[33m';
            const statusIcon = d.status === 'accepted' ? '✓' :
                              d.status === 'superseded' ? '⟳' :
                              d.status === 'deprecated' ? '✗' :
                              '?';
            console.log(`  ${statusColor}${statusIcon}\x1b[0m ${d.id.slice(0, 8)}: ${d.question.slice(0, 50)}${d.question.length > 50 ? '...' : ''}`);
            console.log(`      \x1b[36m→ ${d.choice}\x1b[0m`);
          }
          console.log('');
        }
...
storyCommand
  ...
  .option('-A, --include-archived', 'Include archived stories in listing')
  .action(async (options) => {
    try {
      // Build filters
      const filters: { featureId?: string; status?: StoryStatus } = {};
      ...
      if (options.status) {
        const status = options.status.toLowerCase().replace(/-/g, '_') as StoryStatus;
        if (!VALID_STATUSES.includes(status)) {
          error(`Invalid status: ${options.status}. Valid values: ${VALID_STATUSES.join(', ')}`);
          process.exit(1);
        }
      ...
      // Fetch stories
      const stories = storyRepository.findAll(filters, queryOptions);
  ...
  .action(async (code: string, options) => {
    try {
      ...
      if (options.complexity !== undefined) {
        input.estimatedComplexity = options.complexity || null;
      }
