...
import { createTestDb, TABLES, SCHEMA_VERSION } from '../index';
...
describe('Database Schema', () => {
  ...
  describe('Initial Migration', () => {
    ...
    it('should record migration versions', () => {
      const result = db
  ...
  describe('Features Table', () => {
    ...
    it('should enforce unique code constraint', () => {
      db.run(
        `INSERT INTO ${TABLES.FEATURES} (id, code, name) VALUES ('1', 'TEST', 'Test Feature')`
      );

      expect(() => {
        db.run(
          `INSERT INTO ${TABLES.FEATURES} (id, code, name) VALUES ('2', 'TEST', 'Another Feature')`
        );
      }).toThrow();
    });
    ...
    it('should insert feature with defaults', () => {
      db.run(
        `INSERT INTO ${TABLES.FEATURES} (id, code, name) VALUES ('feat-1', 'NOTIFY', 'Notifications')`
      );

      const feature = db
        .query(`SELECT * FROM ${TABLES.FEATURES} WHERE id = ?`)
        .get('feat-1') as Record<string, unknown>;
  ...
  describe('Stories Table', () => {
    ...
    it('should enforce unique code constraint', () => {
      db.run(
        `INSERT INTO ${TABLES.STORIES} (id, code, feature_id, title) VALUES ('1', 'NOTIFY-001', 'feat-1', 'Story 1')`
      );

      expect(() => {
        db.run(
          `INSERT INTO ${TABLES.STORIES} (id, code, feature_id, title) VALUES ('2', 'NOTIFY-001', 'feat-1', 'Story 2')`
        );
      }).toThrow();
    });
    ...
    it('should enforce foreign key constraint', () => {
      expect(() => {
        db.run(
          `INSERT INTO ${TABLES.STORIES} (id, code, feature_id, title) VALUES ('1', 'FAKE-001', 'non-existent', 'Story')`
        );
      }).toThrow();
    });
    ...
    it('should cascade delete when feature is deleted', () => {
      db.run(
        `INSERT INTO ${TABLES.STORIES} (id, code, feature_id, title) VALUES ('story-1', 'NOTIFY-001', 'feat-1', 'Story 1')`
      );
    ...
    it('should insert story with defaults', () => {
      db.run(
        `INSERT INTO ${TABLES.STORIES} (id, code, feature_id, title) VALUES ('story-1', 'NOTIFY-001', 'feat-1', 'Test Story')`
      );

      const story = db
        .query(`SELECT * FROM ${TABLES.STORIES} WHERE id = ?`)
        .get('story-1') as Record<string, unknown>;
  ...
  describe('Tasks Table', () => {
    ...
    it('should insert task with defaults', () => {
      db.run(
        `INSERT INTO ${TABLES.TASKS} (id, story_id, title) VALUES ('task-1', 'story-1', 'Test Task')`
      );
  ...
  describe('Acceptance Criteria Table', () => {
    ...
    it('should insert acceptance criteria with defaults', () => {
      db.run(
        `INSERT INTO ${TABLES.ACCEPTANCE_CRITERIA} (id, story_id, code, description) VALUES ('ac-1', 'story-1', 'AC-001', 'Test criterion')`
      );

      const ac = db
        .query(`SELECT * FROM ${TABLES.ACCEPTANCE_CRITERIA} WHERE id = ?`)
        .get('ac-1') as Record<string, unknown>;
  ...
  describe('History Table', () => {
    ...
    it('should insert history entry', () => {
      db.run(
        `INSERT INTO ${TABLES.HISTORY} (id, entity_type, entity_id, action, actor, summary) VALUES ('hist-1', 'story', 'story-1', 'created', 'cli', 'Created story')`
      );

      const entry = db
        .query(`SELECT * FROM ${TABLES.HISTORY} WHERE id = ?`)
        .get('hist-1') as Record<string, unknown>;
  ...
  describe('Sessions Table', () => {
    ...
    it('should insert session with defaults', () => {
      db.run(
        `INSERT INTO ${TABLES.SESSIONS} (id, actor) VALUES ('sess-1', 'backend-dev')`
      );
    ...
    it('should set null on story delete (not cascade)', () => {
      db.run(
        `INSERT INTO ${TABLES.SESSIONS} (id, actor, active_story_id, active_task_id) VALUES ('sess-1', 'backend-dev', 'story-1', 'task-1')`
      );
