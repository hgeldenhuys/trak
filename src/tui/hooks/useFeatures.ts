 ...
 * ```
 */
export function useFeatures(): MultiTableQueryResult<Feature[]> {
  return useMultiTableQuery(
    () => featureRepository.findAll(),
    ['feature', 'story']  // Update when stories change (for counts)
  );
}
 ...
 * ```
 */
export function useFeature(featureId: string): MultiTableQueryResult<Feature | null> {
  return useMultiTableQuery(
 ...
 * ```
 */
export function useFeatureByCode(code: string): MultiTableQueryResult<Feature | null> {
  return useMultiTableQuery(
    () => featureRepository.findByCode(code),
    ['feature']
  );
}
