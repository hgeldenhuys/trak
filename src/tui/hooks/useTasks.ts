 ...
 * ```
 */
export function useTasks(options: UseTasksOptions = {}): MultiTableQueryResult<Task[]> {
  return useMultiTableQuery(
    () => taskRepository.findAll(options),
    ['task']
  );
}
 ...
 * ```
 */
export function useTasksByStory(storyId: string): MultiTableQueryResult<Task[]> {
  return useMultiTableQuery(
 ...
 * ```
 */
export function useTasksByStatus(status: TaskStatus): MultiTableQueryResult<Task[]> {
  return useMultiTableQuery(
 ...
 * ```
 */
export function useTask(taskId: string): MultiTableQueryResult<Task | null> {
  return useMultiTableQuery(
 ...
 * ```
 */
export function useTaskStatusCounts(storyId: string): MultiTableQueryResult<Record<string, number>> {
  return useMultiTableQuery(
    () => taskRepository.getStatusCounts(storyId),
    ['task']
  );
}
